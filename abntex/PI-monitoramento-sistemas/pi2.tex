...
\begin{resumo}
    A crescente dependência de ambientes computacionais torna a indisponibilidade e os incidentes de segurança fatores críticos para a continuidade dos serviços. Este trabalho apresenta a fundamentação teórica, o planejamento arquitetural e a implementação de um sistema de monitoramento para servidores Linux, com foco na coleta de métricas de infraestrutura e na detecção de eventos básicos de segurança por meio da análise de logs. A solução foi construída sobre uma arquitetura cliente-servidor: agentes instalados nos dispositivos coletam dados de CPU, memória, disco e rede e os enviam a um servidor central via API REST. No servidor, as informações são processadas, armazenadas em banco de dados relacional e exibidas em um painel \textit{web}. O sistema também identifica padrões suspeitos,  como tentativas de autenticação malsucedidas e varreduras de portas, e envia alertas automáticos por Microsoft Teams e e-mail. O sistema foi operado em rede local controlada com múltiplos clientes Linux reais, demonstrando a viabilidade prática da integração entre monitoramento de infraestrutura e análise de eventos de segurança.
...
Para atingir esses objetivos, o sistema foi construído sobre uma arquitetura cliente-servidor: agentes instalados nos dispositivos coletam os dados localmente e os enviam a um servidor central, responsável pelo processamento e armazenamento.
...
Conforme falado no capítulo~\ref{cap:data-collect} agentes de monitoramento são programas instalados nos dispositivos visando a coleta de informações sobre o dispositivo. Esses agentes atuam diretamente no sistema operacional, capturando métricas como uso de CPU, memória, disco e processos em execução, sendo essenciais para o acompanhamento da infraestrutura de TI.

\citeonline[p.~13]{santos2019monitoramento}, afirma “os agentes de monitoramento são programas executados nos dispositivos que coletam informações e as disponibilizam para sistemas gerenciadores”. Conforme, eles funcionam como intermediários entre os dispositivos monitorados e os sistemas centrais, garantindo que os dados sejam coletados de forma organizada e confiável.

O funcionamento desses agentes baseia-se na coleta contínua ou sob demanda de dados, que são posteriormente enviados para um servidor central para análise. \citeonline[p.~21]{ferreira2020monitoramento}, destaca “os agentes realizam a coleta contínua de dados do sistema e os encaminham para análise”, permitindo o acompanhamento do comportamento dos recursos ao longo do tempo e a identificação de possíveis falhas.
...
Para a detecção de eventos de segurança, o sistema utiliza regras fixas sobre os \textit{logs}, como múltiplas tentativas de autenticação malsucedidas (indicando possível ataque de força bruta) ou conexões em múltiplas portas (indicando varredura de portas). Como foi discutido no Capítulo~\ref{cap:detection}, essa abordagem é eficaz contra ataques conhecidos, mas não detecta comportamentos inéditos. A opção por essa abordagem simplificada é algo exclusivo apenas à esse trabalho e seus objetivos, com o objetivo de reduzir a complexidade de implementação sem comprometer a validação dos princípios de detecção estudados.
...
O controle de versão foi realizado integralmente por meio do Git, com repositório hospedado no GitHub. O histórico de \textit{commits} registra 202 revisões ao longo do período de desenvolvimento, refletindo a evolução incremental de todos os componentes do sistema.

\section{Cronograma e Fases}
\label{sec:cronograma}

O desenvolvimento ocorreu entre março e junho de 2026. Por se tratar de um processo iterativo e incremental, as fases a seguir não são estritamente sequenciais: houve sobreposição entre elas, em especial entre a implementação dos componentes e o refinamento subsequente. O recorte abaixo, reconstituído  partir do histórico de \textit{commits}, organiza o trabalho pela ênfase predominante e cada período.

O trabalho foi norteado por um cronograma inicial de sete semanas (21 de abril a 5 de junho de 2026), organizado por entregas semanais e por componente. A execução acompanhou esse planejamento em linhas gerais, porém com desvios: o arranque efetivo da implementação ocorreu no início de maio, cerca de uma semana após o previsto, e os refinamentos do \textit{frontend} estenderam-se até meados de junho, ultrapassando o congelamento planejado do MVP (5 de junho). Em decorrência desses ajustes, a apresentação final do projeto foi realizada em 26 de junho de 2026.

\begin{itemize}
    \item \textbf{Fase 1: Pesquisa e Especificação}: levantamento bibliográfico, definição da arquitetura e elaboração da fundamentação teórica (Capítulos 1 a 10). Essa etapa corresponde à fase inicial do projeto e antecede o versionamento contínuo do código no repositório atual.
...
inteligência artificial generativa como recurso de apoio de desenvolvimento (não de pesquisa). Este capítulo descreve de forma transparente o escopo e os limites desse uso, em conformidade com as diretrizes de integridade acadêmica.

\section{Ferramenta Utilizada}
\label{sec:ia-ferramenta}

Foi utilizado os assistentes Claude (Anthropic) e DeepSeek, acessado entre maio e junho de 2026
...
falha na arquitetura centralizada. Essas limitações evidenciam o escopo acadêmico do
projeto e mostram claramente o ponto de foco para futuras melhorias do sistema.

Dessa forma, o trabalho cumpre seu papel ao integrar pesquisa teórica, planejamento e
implementação prática de um sistema de monitoramento funcional, servindo como base bem 
estruturada para projetos similares em ambientes acadêmicos e de pequeno porte.

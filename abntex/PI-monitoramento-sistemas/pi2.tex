...
A arquitetura física do sistema é composta por um servidor central e sete clientes, todos conectados a um \textit{switch} não gerenciável em topologia estrela. A rede utiliza a faixa \texttt{10.10.10.0/26} (máscara \texttt{255.255.255.192}), permitindo até 62 endereços IP utilizáveis, com atribuição dinâmica realizada por meio de um serviço DHCP executado no próprio servidor.
...
\begin{itemize}
    ...
    \item \textbf{Frontend (React 18 + Vite + TailwindCSS + react-chartjs-2)}: responsável pela visualização dos dados, consumindo informações via API REST.
\end{itemize}
...
responsabilidades orientada por componente: infraestrutura de rede e configuração do
ambiente, desenvolvimento do agente coletor, desenvolvimento do \textit{backend}
(API REST e lógica de detecção), desenvolvimento do \textit{frontend}, modelagem do
banco de dados e documentação técnica.
% TODO: detalhar papéis individuais: Beatriz, Caio, Denyse, Douglas, Fernando, Larissa, Lucas
...
incrementos ao repositório compartilhado.
% TODO: confirmar se havia reuniões periódicas e com qual frequência

O controle de versão foi realizado integralmente por meio do Git, com repositório
hospedado no GitHub. O histórico de \textit{commits} registra 172 revisões ao longo
...
% TODO: informar duração total e datas de início/fim do projeto (ex.: "quatro meses,
% de fevereiro a junho de 2026").

O desenvolvimento pode ser dividido nas seguintes fases, reconstituídas a partir do
histórico de \textit{commits}:

\begin{itemize}
    \item \textbf{Fase 1 — Pesquisa e Especificação}: levantamento bibliográfico,
          definição da arquitetura e elaboração dos Capítulos 1 a 10 deste documento.
    ...
    \item \textbf{Fase 5 — Validação}: execução dos cenários de teste e documentação
          dos resultados.
\end{itemize}

% TODO: complementar com datas específicas de início de cada fase
...
\begin{itemize}
    ...
    \item \textbf{Linguagem do \textit{frontend}}: JavaScript com React 18, Vite
          (\textit{build tool}), TailwindCSS e react-chartjs-2.
    ...
    % TODO: informar IDE(s) utilizadas — descomente e preencha:
    % \item \textbf{IDE}: VS Code / PyCharm / outro
    % TODO: informar ferramentas de gestão — descomente e preencha:
    % \item \textbf{Ferramentas de gestão}: GitHub Projects / Notion / WhatsApp / outro
\end{itemize}
...
% TODO: inserir figura da arquitetura implementada (diagrama de blocos)
...
O Nginx atua como ponto de entrada único: requisições ao caminho \texttt{/api/*} são
encaminhadas ao contêiner \textit{backend}, enquanto as demais são servidas pelo
contêiner \textit{frontend}. O servidor responde exclusivamente em HTTP, sem
...
inicialização, executa 8 funções \texttt{garantir\_schema\_*()} que realizam migrações
...
\textit{timestamp} nos últimos 2 minutos para o mesmo par (host, IP) —
o \textit{cooldown} — evitando alertas repetidos em varreduras prolongadas.
...
React 18 com Vite como ferramenta de \textit{build} e TailwindCSS para estilização.
A interface exibe dados em seis telas: Dashboard, Métricas, Logs, Alertas,
...
As telas de Alertas e Logs realizam atualizações automáticas a cada 15 e 10
segundos, respectivamente, por meio de \texttt{setInterval}. As demais telas exibem
os dados carregados no momento da navegação ou da troca de \textit{host} selecionado.
...
\texttt{app\_settings}. As cinco primeiras mantêm relação de 1 para N com a tabela
\texttt{host}. A tabela \texttt{app\_settings} é global, armazenando configurações do
sistema independentemente de \textit{host}.

\subsection{Modelo Lógico}
\label{subsec:modelo-logico}

% TODO: inserir figura do modelo lógico gerado externamente a partir do init.sql
% \begin{figure}[htb]
% \legenda[fig:modelo-logico]{Modelo lógico do banco de dados}
% \fig{width=1.00\textwidth}{../img/modelo-logico.png}
% \fonte{Os autores (2026).}
% \end{figure}
...
% ATENÇÃO: Este capítulo deve ser preenchido após a execução do checklist de testes.
% Nenhum número, gráfico ou tabela de resultado deve ser adicionado sem dados reais medidos.
% A latência de detecção é sempre medida pela coluna alerts."timestamp" do banco de dados,
% NÃO pelo momento em que o alerta aparece no painel (que tem polling de 10–15 s e
% introduz atraso artificial na medição).

\section{Ambiente e Cenários de Teste}
\label{sec:ambiente-teste}

Os resultados apresentados neste capítulo foram obtidos em um ambiente de rede local
controlado, com configuração descrita no Capítulo~\ref{cap:implementacao}.
A correta interpretação dos dados de latência pressupõe que os relógios de todos os
hosts estejam sincronizados; o desvio máximo aceitável é inferior a 1 segundo.

\begin{enumerate}
    \item \textbf{Topologia dos hosts monitorados}\\
          Número de hosts monitorados: \textbf{[A PREENCHER]}\ \_\_\_ \\
          Sistema operacional dos alvos: \textbf{[A PREENCHER]}\ \_\_\_ \\
          % TODO: alvo — 7 VMs + 1 servidor físico Ubuntu Server
    \item \textbf{Host atacante}\\
          Sistema operacional e versão: \textbf{[A PREENCHER]}\ \_\_\_ \\
          Posição na rede (mesma sub-rede / roteada): \textbf{[A PREENCHER]}\ \_\_\_
    \item \textbf{Versões das ferramentas de ataque}\\
          \texttt{hydra}: \textbf{[A PREENCHER]}\ \_\_\_ \quad
          \texttt{nmap}: \textbf{[A PREENCHER]}\ \_\_\_ \quad
          \texttt{stress}: \textbf{[A PREENCHER]}\ \_\_\_
    \item \textbf{Sincronização de relógios (NTP)}\\
          Resultado de \texttt{timedatectl} no servidor e em um cliente:\\
          \textbf{[A PREENCHER]}\ \_\_\_
\end{enumerate}

\section{Validação da Coleta de Métricas}
\label{sec:valid-metricas}

Esta seção valida se os valores reportados pelo agente correspondem ao estado real do
sistema monitorado. Para cada métrica, compara-se a leitura do agente com a de uma
ferramenta nativa do sistema operacional coletada simultaneamente. A latência de
chegada ao banco é medida como a diferença entre o \textit{timestamp} do ciclo do
agente e o \textit{timestamp} do registro gravado na tabela \texttt{metrics}.

\begin{enumerate}
    \item \textbf{CPU (\texttt{cpu\_percent} vs. \texttt{top}/\texttt{htop})}\\
          Hosts testados: \textbf{[A PREENCHER]}\ \_\_\_ \\
          Valor reportado pelo agente (\%): \textbf{[A PREENCHER]}\ \_\_\_ \\
          Valor nativo (\%): \textbf{[A PREENCHER]}\ \_\_\_ \\
          Diferença absoluta / percentual: \textbf{[A PREENCHER]}\ \_\_\_
    \item \textbf{Memória (\texttt{memory\_percent} vs. \texttt{free -m})}\\
          Valor reportado / nativo / diferença: \textbf{[A PREENCHER]}\ \_\_\_
    \item \textbf{Disco (\texttt{disk\_percent} vs. \texttt{df -h})}\\
          Valor reportado / nativo / diferença: \textbf{[A PREENCHER]}\ \_\_\_
    \item \textbf{Latência de chegada ao banco}\\
          Média ± faixa (N medições): \textbf{[A PREENCHER]}\ \_\_\_ ms
\end{enumerate}

\section{Validação da Detecção de Força Bruta}
\label{sec:valid-bruteforce}

Esta seção valida a regra de detecção de força bruta SSH: 5 ou mais tentativas
malsucedidas do mesmo IP em uma janela de 60 segundos devem gerar exatamente um
alerta. A latência de detecção é medida como a diferença entre o campo
\texttt{alerts."timestamp"} no banco e o \textit{timestamp} da 5ª tentativa
malsucedida registrada — \textbf{não} pelo momento em que o alerta aparece no painel.
Ferramenta utilizada: \texttt{hydra -l root -P <wordlist> ssh://10.10.10.X}.

\begin{enumerate}
    \item \textbf{Teste de fronteira inferior (4 falhas em 60 s — sem alerta)}\\
          Resultado esperado: nenhum alerta criado.\\
          \textbf{[A PREENCHER]} resultado observado: \_\_\_
    \item \textbf{Teste de fronteira superior (5 falhas em 60 s — alerta disparado)}\\
          Resultado esperado: alerta do tipo \texttt{brute\_force} criado.\\
          \textbf{[A PREENCHER]} resultado observado: \_\_\_
    \item \textbf{Latência de detecção} (5 execuções independentes, aguardar resolução
          manual entre cada uma)\\
          Latência$_i$ = \texttt{alerts.timestamp}$_i$ $-$ $t_{\text{disparo},i}$\\
          Execuções (ms): \textbf{[A PREENCHER]}\ \_\_\_ ; \_\_\_ ; \_\_\_ ; \_\_\_ ; \_\_\_ \\
          Média ± faixa: \textbf{[A PREENCHER]}\ \_\_\_
    \item \textbf{Deduplicação} (tentativas contínuas com alerta ativo)\\
          Resultado esperado: segundo alerta \textbf{não} criado enquanto o primeiro
          permanecer ativo.\\
          \textbf{[A PREENCHER]} resultado observado: \_\_\_
    \item \textbf{Falso positivo} (login legítimo com sucesso)\\
          Resultado esperado: nenhum alerta.\\
          \textbf{[A PREENCHER]} resultado observado: \_\_\_
    \item \textbf{Latência das notificações} (executar após confirmar alerta no banco)\\
          Latência Teams = \texttt{timestamp\_mensagem} $-$ \texttt{alerts.timestamp}:\\
          \textbf{[A PREENCHER]}\ \_\_\_ ms\\
          Latência e-mail = \texttt{timestamp\_recebimento} $-$ \texttt{alerts.timestamp}:\\
          \textbf{[A PREENCHER]}\ \_\_\_ ms
\end{enumerate}

\section{Validação da Detecção de Port Scan}
\label{sec:valid-portscan}

Esta seção valida a detecção de varredura de portas no agente: 10 ou mais portas
distintas de um mesmo IP dentro de uma janela de 60 segundos devem gerar um alerta.
O \textit{cooldown} de 2 minutos impede alertas repetidos para o mesmo IP; os testes
de latência devem ser espaçados por pelo menos 3 minutos para evitar supressão.
A latência é medida pelo campo \texttt{alerts."timestamp"} no banco. Ferramenta
utilizada: \texttt{nmap}.

\begin{enumerate}
    \item \textbf{Teste de fronteira inferior (9 portas em 60 s — sem alerta)}\\
          Comando: \texttt{nmap -sS -{}-top-ports 9 10.10.10.X}\\
          Resultado esperado: nenhum alerta.\\
          \textbf{[A PREENCHER]} resultado observado: \_\_\_
    \item \textbf{Teste de fronteira superior (11 portas em 60 s — alerta disparado)}\\
          Comando: \texttt{nmap -sS -{}-top-ports 11 10.10.10.X}\\
          Resultado esperado: alerta do tipo \texttt{port\_scan} criado.\\
          \textbf{[A PREENCHER]} resultado observado: \_\_\_
    \item \textbf{Latência de detecção} (5 execuções, intervalo $>$ 3 min entre cada)\\
          Latência$_i$ = \texttt{alerts.timestamp}$_i$ $-$ $t_{\text{disparo},i}$\\
          Execuções (ms): \textbf{[A PREENCHER]}\ \_\_\_ ; \_\_\_ ; \_\_\_ ; \_\_\_ ; \_\_\_ \\
          Média ± faixa: \textbf{[A PREENCHER]}\ \_\_\_
    \item \textbf{Cooldown} (segundo scan imediato após alerta)\\
          Resultado esperado: segundo alerta \textbf{não} criado dentro de 2 minutos.\\
          \textbf{[A PREENCHER]} resultado observado: \_\_\_
    \item \textbf{Tipos de scan} (\texttt{-sS}, \texttt{-sT}, \texttt{-sF})\\
          \begin{tabular}{lc}
          \hline
          Tipo & Detectado? (s/n) \\
          \hline
          \texttt{nmap -sS} (SYN scan) & \textbf{[A PREENCHER]}\ \_\_\_ \\
          \texttt{nmap -sT} (Connect scan) & \textbf{[A PREENCHER]}\ \_\_\_ \\
          \texttt{nmap -sF} (FIN scan) & \textbf{[A PREENCHER]}\ \_\_\_ \\
          \hline
          \end{tabular}
    \item \textbf{Degradação graciousa — sem \texttt{tcpdump}}\\
          Remover \texttt{tcpdump} temporariamente e reiniciar o agente.\\
          Resultado esperado: agente continua em execução; nenhum erro fatal no \textit{log}.\\
          \textbf{[A PREENCHER]} resultado observado: \_\_\_
    \item \textbf{Falso positivo} (tráfego SYN legítimo, ex.: navegação web)\\
          Resultado esperado: nenhum alerta gerado.\\
          \textbf{[A PREENCHER]} resultado observado: \_\_\_
\end{enumerate}

\section{Validação do Pipeline de Alertas e Notificações}
\label{sec:valid-alertas}

Esta seção valida o caminho completo de um alerta: do evento disparador até a
notificação ao operador. A latência fim-a-fim é decomposta em dois segmentos:
(a) detecção (evento $\rightarrow$ \texttt{alerts.timestamp}), medida nas seções
anteriores; e (b) notificação (\texttt{alerts.timestamp} $\rightarrow$ recebimento
da mensagem). Os testes de notificação verificam também a funcionalidade dos
\textit{toggles} de canais na tabela \texttt{app\_settings}.

\begin{enumerate}
    \item \textbf{Teams habilitado — alerta \texttt{brute\_force}}\\
          Latência Teams = \texttt{timestamp\_mensagem} $-$ \texttt{alerts.timestamp}:\\
          \textbf{[A PREENCHER]}\ \_\_\_ ms
    \item \textbf{E-mail habilitado — alerta \texttt{brute\_force}}\\
          Latência e-mail = \texttt{timestamp\_recebimento} $-$ \texttt{alerts.timestamp}:\\
          \textbf{[A PREENCHER]}\ \_\_\_ ms
    \item \textbf{Toggle \texttt{notify\_email = false}}\\
          Resultado esperado: alerta gravado no banco; e-mail \textbf{não} enviado.\\
          \textbf{[A PREENCHER]} resultado observado: \_\_\_
    \item \textbf{Toggle \texttt{notify\_teams = false}}\\
          Resultado esperado: alerta gravado; mensagem Teams \textbf{não} recebida.\\
          \textbf{[A PREENCHER]} resultado observado: \_\_\_
    \item \textbf{Latência fim-a-fim (força bruta, Teams habilitado)}\\
          Latência total = \texttt{timestamp\_mensagem} $-$ $t_{\text{disparo}}$:\\
          \textbf{[A PREENCHER]}\ \_\_\_ ms
\end{enumerate}

\section{Desempenho e Overhead do Agente}
\label{sec:overhead}

Esta seção quantifica o impacto do agente sobre o sistema monitorado. As medições
de CPU e memória utilizam \texttt{psutil} no próprio host e são coletadas durante
operação em regime estacionário (após o primeiro ciclo completo de \textit{discovery}).
São realizadas duas séries: com \texttt{tcpdump} ativo e sem \texttt{tcpdump}, para
isolar a contribuição da captura de pacotes.

\begin{enumerate}
    \item \textbf{CPU do processo agente em regime normal}\\
          Com \texttt{tcpdump}: \textbf{[A PREENCHER]}\ \_\_\_ \% \\
          Sem \texttt{tcpdump}: \textbf{[A PREENCHER]}\ \_\_\_ \%
    \item \textbf{Memória RSS do processo agente}\\
          Com \texttt{tcpdump}: \textbf{[A PREENCHER]}\ \_\_\_ MB \\
          Sem \texttt{tcpdump}: \textbf{[A PREENCHER]}\ \_\_\_ MB
    \item \textbf{Tempo médio de ciclo de 5 s}\\
          Medido entre início e fim do laço principal (N amostras):\\
          \textbf{[A PREENCHER]}\ \_\_\_ ms (média) ± \_\_\_ ms (desvio)
    \item \textbf{Tamanho médio do \textit{payload} de métricas (JSON)}\\
          \textbf{[A PREENCHER]}\ \_\_\_ KB
    \item \textbf{Comportamento da fila de \textit{retry} após queda do \textit{backend}}\\
          Procedimento: derrubar o contêiner \textit{backend} por 30 s e restaurar.\\
          Resultado esperado: agente enfileira requisições; envia ao recuperar.\\
          \textbf{[A PREENCHER]} resultado observado: \_\_\_ ; itens na fila: \_\_\_
\end{enumerate}

\section{Discussão dos Resultados}
\label{sec:discussao}

% TODO: redigir após consolidar os dados das Seções 13.1–13.6.
% Estrutura sugerida:
%  - Confrontar cada objetivo específico do Cap. 1 com os resultados:
%    (i)  coleta de métricas  — Seção~\ref{sec:valid-metricas}
%    (ii) processamento/armazenamento — inferido dos testes anteriores
%    (iii) detecção de força bruta e port scan — Seções~\ref{sec:valid-bruteforce}
%          e~\ref{sec:valid-portscan}
%    (iv) visualização — painel funcional, cf. Cap. 12
%  - Discutir overhead observado, falsos positivos e limitações práticas dos testes.
%  - Indicar se algum resultado foi inesperado e como isso impacta as conclusões.
...
\begin{itemize}
    ...
    \item \textbf{Lacunas no frontend}: três comportamentos estão identificados como
          pendentes de correção: falha ao clicar em sub-abas; contador de alertas
          na barra lateral com valor fixo; métricas sem atualização automática
          periódica.
\end{itemize}
...
% =================================================================================================
% 16. USO DE INTELIGÊNCIA ARTIFICIAL NO TRABALHO
...
inteligência artificial. Todo conteúdo gerado pelo assistente foi revisado
criticamente pelos autores e verificado contra o código-fonte do repositório antes
de ser incorporado a este documento. Os autores assumem responsabilidade integral
pelo conteúdo aqui apresentado.

% =================================================================================================
% APÊNDICE A — USO DE FERRAMENTAS DE IA NO PROJETO
% CONDICIONAL: incluir somente após confirmação da política da Universidade Tuiuti
% do Paraná e do orientador sobre uso de IA generativa em projetos interdisciplinares.
% =================================================================================================
%
% \begin{apendicesenv}
% \partapendices
%
% \chapter{Uso de Ferramentas de Inteligência Artificial no Projeto}
% \label{ap:ia}
%
% Durante o desenvolvimento deste projeto, foram utilizadas ferramentas de
% inteligência artificial generativa como auxílio à documentação técnica e à
% elaboração deste documento. O presente apêndice descreve o escopo desse uso.
%
% \section{Ferramentas Utilizadas}
%
% O assistente Claude Code (Anthropic) foi utilizado nas seguintes atividades:
%
% \begin{itemize}
%     \item Análise e verificação de consistência entre o código-fonte implementado
%           e o texto acadêmico;
%     \item Geração de rascunhos dos Capítulos 11--15, revisados pelos autores;
%     \item Identificação de contradições entre o planejamento (Caps. 1--10) e a
%           implementação real.
% \end{itemize}
%
% \section{Limites do Uso}
%
% Todo conteúdo técnico foi verificado contra o código-fonte do repositório.
% Nenhuma referência bibliográfica foi inventada; todas as citações utilizam
% exclusivamente entradas pré-existentes em \texttt{referencias.bib}. Os resultados
% do Capítulo~\ref{cap:resultados} são baseados exclusivamente em dados medidos
% pela equipe.
%
% \end{apendicesenv}

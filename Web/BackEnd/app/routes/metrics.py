    ...
    def post_metrics():
        ...
        try:
            ...
            if AlertModel is not None and check_resource_alert_fn is not None:
                ...
                check_resource_alert_fn(db, AlertModel, host.id, 'cpu_high',  normalized['cpu_percent']    or 0.0, _hn, _hip)
                check_resource_alert_fn(db, AlertModel, host.id, 'mem_high',  normalized['memory_percent'] or 0.0, _hn, _hip)
                check_resource_alert_fn(db, AlertModel, host.id, 'disk_high', normalized['disk_percent']   or 0.0, _hn, _hip)

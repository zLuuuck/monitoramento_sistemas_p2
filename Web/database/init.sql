...
CREATE INDEX idx_metrics_high_cpu ON metrics (host_id, timestamp)
WHERE cpu_percent > 85;
CREATE INDEX idx_metrics_high_mem ON metrics (host_id, timestamp)
WHERE memory_percent > 90;
...
CREATE INDEX idx_logs_auth_ip ON logs ((parsed_data->>'ip_origem'))
WHERE log_type = 'auth';
CREATE INDEX idx_logs_auth_failed ON logs (timestamp)
WHERE log_type = 'auth'
...
CREATE INDEX idx_conn_duration ON active_connections (duration_sec)
WHERE duration_sec > 3600;
-- ============================================================
-- POLÍTICA DE RETENÇÃO (7 dias)
-- ============================================================
CREATE OR REPLACE FUNCTION cleanup_old_data(days_to_keep INTEGER DEFAULT 7) RETURNS VOID AS $$ BEGIN
DELETE FROM metrics
